class Key {
    ...
    static parse( string ) {
        ...
        string.split( /[+-]/g ).forEach( part => { switch ( part ) {
            case 'C' : key.control = true; break ;
            case 'M' : key.meta = true; break ;
            case 'S' : key.shift = true; break ;
            default : key.key = part; break;
        } } );
...
export class KeySequence {
    ...
    constructor( sequence ) {
        ...
        this._sequence = sequence.split( /\s+/g )
    ...
    match( event ) {
        ...
        for ( var t = 0, T = this._sequence.length; t < T; ++ t )
